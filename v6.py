...
st.set_page_config(
    page_title="UVXY Lead → TSLA Trade Alert v2",
    layout="wide",
    page_icon="⚡"
)
...
st.markdown(
    "**UVXY 作為領先指標 • 偵測不同步 • 明確買/賣 TSLA 建議 • "
    "已加入最小實體過濾與大K線優先**"
)
...
with st.sidebar:
    st.header("⚙️ 設定")
    n_candles = st.slider("回顧根數 (N)", 3, 15, 7)
    trend_threshold = st.slider("趨勢強度門檻 (%)", 60, 90, 70)
    min_body_pct = st.slider("最小實體過濾 (%)", 0.05, 0.5, 0.15)
    big_candle_pct = st.slider("大K線強制門檻 (%)", 0.3, 1.5, 0.5)
    cooldown_min = st.slider("Telegram 冷卻時間 (分鐘)", 1, 15, 5)
    auto_refresh = st.checkbox("每 60 秒自動更新", value=True)
...
try:
    ...
    st.error("請在 .streamlit/secrets.toml 中設定 telegram.bot_token 與 chat_id")
    st.stop()
...
if uvxy_trend == "UP":
    tsla_follow_expected = tsla_net_pct <= -0.12
...
if desync and (now - st.session_state.last_alert).total_seconds() > cooldown_min * 60:
    ...
    if len(tsla_recent) >= 2:
        ...
        stop_ref = (
            f"建議停損參考："
            f"{'前低 ' + f'{prev_low:.2f}' if uvxy_trend == 'UP' else '前高 ' + f'{prev_high:.2f}'}"
        )
...
目前價格：
  TSLA ${tsla_latest['Close']:.2f}   
  UVXY ${uvxy_latest['Close']:.2f}
...
# ────────────────────────────────────────────────────────────────
cols = st.columns(5)
cols[0].metric("TSLA 最新價", f"${tsla_latest['Close']:.2f}",
               f"{tsla_latest['Close'] - tsla_df['Close'].iloc[-2]:+.2f}")
cols[1].metric("UVXY 最新價", f"${uvxy_latest['Close']:.2f}",
               f"{uvxy_latest['Close'] - uvxy_df['Close'].iloc[-2]:+.2f}")
...
fig = make_subplots(
    rows=1, cols=2,
    subplot_titles=("TSLA 最近 15 分鐘", "UVXY 最近 15 分鐘")
)

fig.add_trace(
    go.Candlestick(
        x=last15_tsla.index,
        open=last15_tsla.Open, high=last15_tsla.High,
        low=last15_tsla.Low, close=last15_tsla.Close,
        name="TSLA",
...
fig.add_trace(
    go.Candlestick(
        x=last15_uvxy.index,
        open=last15_uvxy.Open, high=last15_uvxy.High,
        low=last15_uvxy.Low, close=last15_uvxy.Close,
        name="UVXY",
        increasing_line_color="#ffaa00",
        decreasing_line_color="#ff4d4d"
    ),
    row=1, col=2
)
...
if len(corr_hist) > 5:
    ...
    fig_corr.add_hline(
        ...
        annotation_text="-0.5 警戒線"
    )
    fig_corr.update_layout(
        height=340,
        template="plotly_dark",
        title="Pearson 相關係數走勢 (最近 60 分鐘滾動)",
        yaxis_title="相關係數"
    )
